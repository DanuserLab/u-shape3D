function weinerEstimateList = weinerDeconvolveFull(MD, PSF, maxOTF, imageBlurWeiner, morphRadiusErode, morphRadiusDilate, imageMultiply, p, photoDir, imageSaveDir)
...
% calculate the OTF (optical transfer function)
disp('  Calculating the OTF')
[OTF, newImageSize] = calculateSmallOTF(PSF, imageSize); clear PSF;

% calculate an apodization filter
disp('  Calculating the apodization filter')
apodizeFilter = makeApodizationFilter(OTF, maxOTF, p.apoHeight);
...
for t = 1:MD.nFrames_    
    for c = p.chanList
        ...
        image3D = weinerDeconvolve(image3D, OTF, weinerEstimate, apodizeFilter);
