...
% make multiple loops
% pull out registration, so that it's done first in a different loop
% save all of the tforms in a structure

% in the next loop, load all of the vertices and apply tform, keep all the
% vertices around.
ip = inputParser;
...
%         sNew = load(sprintf(meshPath, c, t));
        sNew = load(meshPath);
