...
% initiate a directory to store intensity-protrusion analyses in
...
%         % load the raw image (for the debug plots)
...
%                 image3D = iStruct.imageSurface;
%                 cmap = colormap(parula);
%                 climits = [min(motionS) max(motionS)];
%                 light_handle = plotMeshFigure(image3D, surface, motionS, cmap, climits);
%                 title('Motion')
%             end
...
%             1;
        ...
        if p.calculateVonMises == 1
            ...
            [~, cellCenter] = findInteriorMostPoint(imageMasked);
