function [meanVM, concVM] = estimateVonMisesFisherParameters(unitVectors, dim)

% estimateVonMisesFisherParameters - for a set of unit vectors, estimate the the von Mises Fisher parameters in the given dimensions
%
...
meanNonNormal = sum(unitVectors, 1);
normalization = sqrt(sum(meanNonNormal.^2));
meanVM = meanNonNormal./normalization;

% calculate an approximation of the the maximum likelihood estimate of the concentration parameter
