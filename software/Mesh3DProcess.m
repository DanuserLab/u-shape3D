...
% 
    properties (Constant)
        validMeshModes =         {'otsu', 'otsuMulticell', 'otsuSteerable',...
                                  'stdSteerable', 'twoLevelSurface', 'threeLevelSurface', ...
                                  'threeLevelSurfaceMultiApo', 'readDaeFile', 'loadMask', 'activeContour'};
        validRegistrationModes = {'translation', 'rigid', 'affine'};
        validSmoothMeshModes =   {'curvature', 'none'};
        guiIgnoreParameters = {'daeFilePath', 'maskDir', 'maskName'};
    end
    ...
    methods (Access = public)
        ...
        function checkParameters(obj, owner, params)
            ...
            addParameter(ip,'registrationMode',@ischar);
            addParameter(ip,'saveRawImages',@(x) (x==0 || x==1));
            ...
            addParameter(ip,'saveCurvatureImage',@(x) (x==0 || x==1));
            addParameter(ip,'saveEdgeImage',@(x) (x==0 || x==1));
            addParameter(ip,'saveRawSubtractedImages',@(x) (x==0 || x==1));
            ...
            addParameter(ip,'daeFilePath',@(x) (isempty(x) || ischar(x)));
            addParameter(ip,'maskDir',@(x) (isempty(x) || ischar(x)));
            addParameter(ip,'maskName',@(x) (isempty(x) || ischar(x)));
            ...
            % meshModes = {'otsu', 'otsuMulticell', 'otsuSteerable', 'stdSteerable', 'twoLevelSurface', 'threeLevelSurface', 'threeLevelSurfaceMultiApo', 'readDaeFile', 'loadMask', 'activeContour'};
            assert(max(cellfun(@(x) strcmp(x,paramsMatched.meshMode), Mesh3DProcess.validMeshModes)), '%s is an invalid meshMode parameter', paramsMatched.meshMode);

            % check the registrationMode parameter
            % registrationModes = {'translation', 'rigid', 'affine'};
            assert(max(cellfun(@(x) strcmp(x,paramsMatched.registrationMode), Mesh3DProcess.validRegistrationModes)), '%s is an invalid registrationMode parameter', paramsMatched.registrationMode);
            
            % check the smoothMeshMode parameter
            % smoothMeshModes = {'curvature', 'none'};
            assert(max(cellfun(@(x) strcmp(x,paramsMatched.smoothMeshMode), Mesh3DProcess.validSmoothMeshModes)), '%s is an invalid smoothMeshMode parameter', paramsMatched.smoothMeshMode);
...
%             data = obj.owner_;
    ...
    methods (Static)
        ...
        function defaultParams = getDefaultParams(owner,varargin)
            ...
            defaultParams.meshMode = {'otsu'}; % modes: 'otsu', 'otsuMulticell', 'otsuSteerable', 'twoLevelSurface', 'threeLevelSurface', 'readDaeFile', 'loadMask'
            defaultParams.registerImages = 0; % set to 1 to register the images prior to meshing
            defaultParams.saveRawImages = 0; % set to 1 to register and save the raw images
            defaultParams.registrationMode = {'translation'}; % type of registration used, if registration is enabled. Options: 'translation', 'rigid', 'affine'
            defaultParams.scaleOtsu = 1; % an Otsu threshold multiplier
            defaultParams.imageGamma = 1; % gamma adjust prior to thresholding
            defaultParams.smoothImageSize = 0; % the std of the Gaussian kernel used to smooth the image
            ...
            defaultParams.saveCurvatureImage = 0;
            defaultParams.saveEdgeImage = 0;
            defaultParams.saveRawSubtractedImages = 0;
            ...
            % parameters for loading externally generated meshes
            defaultParams.daeFilePath = []; % the path to a mesh stored as a dae file. Only used if meshMode is readDaeFile
            
            % parameters for loading an externally generated mask
            defaultParams.maskDir = []; % the directoty of an optional mask to load
            defaultParams.maskName =[]; % the name of an optional mask to load (saved as an image)
