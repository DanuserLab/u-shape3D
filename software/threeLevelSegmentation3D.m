...
% %% old code (closer to an implemntation of Hunter's code)
% 
% %scales = 1.5, 2, 4
% %nSTDintensity = 3; nSTDsurface = 2;
% 
% % add a black border to the image in case the cell touches the border
% image3D = addBlackBorder(image3D, 1);
% 
% % surface filter the image
% q.SigmasXY = scales; q.SigmasZ = scales; q.WeightZ = 1;
% maxResp = multiscaleSurfaceFilter3D(image3D,q);
% 
% % create a background mask
% [backMean, backSTD] = robustMean(double(image3D(:)),[],2);
% backThresh = backMean + nSTDintensity*backSTD;
% backMask = image3D > backThresh;
% 
% % get the foreground threshold
% foreThresh = thresholdOtsu(image3D(backMask(:)));
% foreMask = image3D > foreThresh;
% 
% % get the surface threshold for the response image
% surfBackMean = mean(maxResp(:));
% surfBackSTD = std(maxResp(:));
% surfThresh = surfBackMean + (nSTDsurface*surfBackSTD);
...
% smoothedMask = filterGauss3D(mask, 1);
% level = 0.5;
% smoothedMask = removeDisconectedComponents(smoothedMask, level);
% smoothedMask = imfill(smoothedMask); 
% surface = isosurface(smoothedMask, level);
