function meshColor = loadCurvatureWatershedsSurfaceColor(MD, chan, frame, colorKey)

% loadCurvatureWatershedsSurfaceColor - loads the color of a mesh surface colored by the raw curvature watersheds
%
...
watershedName = ['watersheds_' num2str(chan) '_' num2str(frame) '.mat'];
...
onBoundary = findBoundaryFaces(wStruct.watersheds, nStruct.neighbors, 'double');

% make faces that are on the boundary or outside of a bleb grey
meshColor = mod(wStruct.watersheds*colorKey,1024)+1;
meshColor(wStruct.watersheds<1) = 0; % outside of a bleb
