...
%
%   p.mainInDirectory - the directory where Morphology3D information is saved
% 
...
%     'curvatureSurfaceSegmentPatchMerge' - color the surface by curvature and patch merged surface segmentation
%
%   p.clickMode
...
assert(isdir(p.mainInDirectory), 'mainInDirectory is not a directory');
...
for c = 1:length(p.cellsList)
    ...
    try 
        mdName = dir(fullfile(p.mainInDirectory, p.cellsList{c}, '*.mat'));
        load(fullfile(p.mainInDirectory, p.cellsList{c}, mdName.name));
    catch
    ...
    savePath = fullfile(p.mainInDirectory, p.cellsList{c}, 'TrainingData', p.nameOfClicker);
    if ~isdir(savePath)
        mkdir(savePath)
    else
    ...
    p.chanList = 1:length(MD.channels_); % define channel - HMF
    ch = p.chanList(1); % assign the first channel for motif detection by clicking - HMF
    analysisPath = fullfile(p.mainInDirectory, p.cellsList{c}, 'Morphology', 'Analysis');
    surfacePath = [analysisPath filesep 'Mesh' filesep 'ch' num2str(ch)];
    surfaceSegmentPath = [analysisPath filesep 'SurfaceSegment' filesep 'ch' num2str(ch)];
    surfaceSegmentPatchMergePath = [analysisPath filesep 'PatchMerge' filesep 'ch' num2str(ch)];
    ...
    for f = 1:length(frames) 
       ...
       elseif  strcmp(p.surfaceMode, 'surfaceSegmentPatchMerge')
            ...
            cmap = [0.2, 0.2, 0.2; 0.6,0.6,1.0];
            climits = [0 1];
       elseif strcmp(p.surfaceMode, 'curvatureSurfaceSegmentPatchMerge')
            cStruct = load(fullfile(surfacePath, sprintf('meanCurvature_%i_%i.mat', chan, frames(f))));
            sSStruct = load(fullfile(surfaceSegmentPatchMergePath, sprintf('surfaceSegment_%i_%i.mat', chan, frames(f))));
            nStruct = load(fullfile(surfacePath, sprintf('neighbors_%i_%i.mat', chan, frames(f))));
            onBoundary = findBoundaryFaces(sSStruct.surfaceSegmentPatchMerge, nStruct.neighbors, 'double');
            meshColor = cStruct.meanCurvature*(-1000/MD.pixelSize_);
            cmap = [0,0,0; flipud(makeColormap('div_rwb', 1024))];
            climits = [-1.52 1.5];
            meshColor(meshColor < -1.5) = -1.5;
            meshColor(onBoundary==1) = -1.52; % on the boundary
