function [surface, image3D, level] = combineThreeImagesMeshOtsu(image3D_1, method_1, image3D_2, method_2, image3D_3, method_3, scaleLevel)
...
assert(strcmp(method_3, 'Otsu') | (isscalar(method_3) && method_3>=0), 'method_3 must be set to either "Otsu" or a positive number');
...
% scale the third image by its Otsu threshold
if strcmp(method_3, 'Otsu')
    [image3D_3, level] = prepareCellOtsuSeg(image3D_3);
    level = scaleLevel*level;
    image3D_3 = image3D_3-level;
    image3D_3 = image3D_3/std(image3D_3(:));
else
    level = scaleLevel*method_3;
    image3D_3 = image3D_3-level;
    image3D_3 = image3D_3/std(image3D_3(:));
end

% combine the two images
image3D = max(max(image3D_1, image3D_2), image3D_3);
