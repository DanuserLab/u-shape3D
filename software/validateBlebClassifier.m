...
%     [model, inModel, ~] = svmWithFeatureSelection(trainMeasures, trainBlebs', 1, numSVMreps);
%     %[model, inModel, ~] = forestWithFeatureSelection(trainMeasures, trainBlebs', 1, numSVMreps);
%     
...
%     testPredict = str2num(cell2mat(testPredict));
%     
...
parfor c = 1:length(p.clicksPathList)
...
for n = 1:numCompares
    ...
    assert(2*numTrainingCells+numTestCells <= length(p.clicksPathList), 'the training and test sets are too large to select from the click pathes without overlap');
    rng('shuffle');
    ...
    % train on the training data
    [model1, inModel1, ~] = svmWithFeatureSelection(trainMeasures1, trainBlebs1', numSVMreps);
    [model2, inModel2, ~] = svmWithFeatureSelection(trainMeasures2, trainBlebs2', numSVMreps);
