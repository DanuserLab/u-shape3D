function meshColor = loadSVMscoreSurfaceColor(MD, chan, frame)

% loadSVMscoreSurfaceColor - loads the color of a mesh surface colored by SVM score
%
...
meshColor = sStruct.SVMscore';
% meshColor(meshColor > 4) = 4; meshColor(meshColor < -4) = -4; % this is a hack (-- and looks better!)
meshColor(onBoundary==1) = -1*max(abs(meshColor)) - 0.1; % on the boundary
