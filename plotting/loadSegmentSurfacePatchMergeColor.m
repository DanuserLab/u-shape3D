function meshColor = loadSegmentSurfacePatchMergeColor(MD, chan, frame, colorKey)

% loadSegmentSurfacePatchMergeColor - loads the color of a mesh surface colored by curvature segmentation
%
...
blebPath = MD.findProcessTag('PatchMerge3DProcess',false, false,'tag',false,'last').outFilePaths_{1,chan}; 
% blebPath = [MD.outputDirectory_ filesep 'Morphology' filesep 'Analysis' filesep 'PatchMerge']; 
...
onBoundary = findBoundaryFaces(bStruct.surfaceSegmentPatchMerge, nStruct.neighbors, 'double');

% make faces that are on the boundary or outside of a segment grey
meshColor = mod(bStruct.surfaceSegmentPatchMerge*colorKey,1024)+1;
meshColor(bStruct.surfaceSegmentPatchMerge<1) = 0; % outside of a bleb
